...
genConfig :: (Applicative g, Pick g) => g Config
genConfig =
  Config <$> param
    <*> param
    <*> param
    ...
    <*> param
  where
    param = pick [(1, "t", pure True), (1, "f", pure False)]

isConfig :: Config -> Bool
isConfig c = ((paramA c && paramB c) || paramC c) && (paramD c || not (paramJ c))

instance HasStats Config where
  sizeFn _ = 1
  nodeFreqs _ = Map.empty
  ungenerate _ = ""
  subtrees _ = Set.empty
